...
//    TH1D** histos =gedPhotonAnalyzer(inputFile, "ggHiNtuplizer/EventTree");
//    TH1D** histosGED =gedPhotonAnalyzer(inputFile, "ggHiNtuplizerGED/EventTree");
...
std::vector<TH1*> gedPhotonAnalyzer(TFile* inputFile, const char* treePath)
{
    ...
    for(Long64_t jj = 0; jj < entries; ++jj)
    {
        ...
        // find GEN photons that match to RECO photons
            for (int i=0; i < nPho; ++i)
            {
                ...
                for (int j=0; j<nMC; ++j)
                {
                    deltaRtmp = getDR(mcEta->at(j), mcPhi->at(j), phoEta->at(i), phoPhi->at(i));
            ...
            for(int i = 0; i < nMC; ++i){
                ...
                if(passedGENselection)
                {
                    ...
                    // fill histograms
                    h[6]->Fill(mcPt->at(i));
                    if (isMiss[i] == true)  h[7]->Fill(mcPt->at(i));
                }
            }
        }
    ...
    for(int r=0; r < numGENptBins; ++r){
        ...
        int n = energyScale[2][r];
        double mean = energyScale[0][r]/n;
        double meanOfSquares = energyScale[1][r]/n;
        h[2]->SetBinContent(r+1,mean);
